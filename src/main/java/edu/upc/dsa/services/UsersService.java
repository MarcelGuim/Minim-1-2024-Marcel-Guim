...
public class UsersService {
    ...
    public Response newUser(User u) {
        if (u.getName() == null || u.getSurname() == null || u.getMail() == null || u.getDate() == null) return Response.status(500).entity(u).build();
    ...
    @ApiResponses(value = {
            @ApiResponse(code = 201, message = "Successful", response=Mix.class),
            @ApiResponse(code = 500, message = "Validation Error")
    ...
    public Response AddPointToUser(Mix m) {
        ...
        this.Um.RegisterInterestPoint(m.getU().getId(),m.getP().getHorizontal(),m.getP().getVertical());
        return Response.status(201).entity(m).build();
    }
    @GET
    @ApiOperation(value = "get a Point of a User With Id of the user", notes = "asdasd")
    @ApiResponses(value = {
            @ApiResponse(code = 201, message = "Successful", response=Points.class, responseContainer = "List"),
            @ApiResponse(code = 500, message = "Validation Error")
    })
    ...
    @GET
    @ApiOperation(value = "get a points of a Type", notes = "asdasd")
    @ApiResponses(value = {
            @ApiResponse(code = 201, message = "Successful", response=Points.class, responseContainer = "List"),
            @ApiResponse(code = 500, message = "Validation Error")
    })
    @Path("/pointsoftype/{id}")
    public Response deleteTrack(@PathParam("id") String type) {
